...
import { Mrbr_UI_Bootstrap_Controls_TooltipTriggers } from "../ui/bootstrap/controls/TooltipTriggers";
...
export class Mrbr_Tests_Application$Tooltip {
  constructor() {
    ...
    MrbrBase.mrbrInstance.loadManifest(Mrbr_Tests_Application$Tooltip[MrbrBase.MANIFEST])
      .then(async () => {
        const
          config = new Mrbr_UI_Bootstrap_Controls_TooltipConfig("This is a tooltip a3")
            .Placement(Mrbr_UI_Bootstrap_Controls_TooltipPlacements.bottom)
            .Trigger(Mrbr_UI_Bootstrap_Controls_TooltipTriggers.hover);
