...
function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$Breadcrumb[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {
                let applicationBreadcrumb = new Mrbr.Tests.Application$Breadcrumb();
                applicationBreadcrumb.initialise();
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$Breadcrumb, 0),
