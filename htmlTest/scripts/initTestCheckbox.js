function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$Checkbox[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let checkboxApplication = new Mrbr.Tests.Application$Checkbox()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$Checkbox, 0),
