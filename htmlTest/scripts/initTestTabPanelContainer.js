...
function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$TabPanelContainer[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let application = new Mrbr.Tests.Application$TabPanelContainer()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$TabPanelContainer, 0),
