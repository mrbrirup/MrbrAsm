...
function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$ListGroupTabs[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let DropdownApplication = new Mrbr.Tests.Application$ListGroupTabs()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$ListGroupTabs, 0),
