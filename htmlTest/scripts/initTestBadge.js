...
function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$Badge[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {
                let applicationBadge = new Mrbr.Tests.Application$Badge()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$Badge, 0),
