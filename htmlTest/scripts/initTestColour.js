function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$Colour[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let application = new Mrbr.Tests.Application$Colour()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$Colour, 0),
