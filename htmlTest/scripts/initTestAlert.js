...
function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$Alert[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {
                let applicationAlert = new Mrbr.Tests.Application$Alert()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$Alert, 0),
