...
function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$ToastRack[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let DropdownApplication = new Mrbr.Tests.Application$ToastRack()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$ToastRack, 0),
