function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$Form[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let application = new Mrbr.Tests.Application$Form()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$Form, 0),
