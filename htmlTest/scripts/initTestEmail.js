function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$Email[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let EmailApplication = new Mrbr.Tests.Application$Email()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$Email, 0),
