function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$HorizontalLayout[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let application = new Mrbr.Tests.Application$HorizontalLayout()
            })
...
async function onReady() {
    try {
        window["mrbrLoadManifest"] =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$HorizontalLayout, 0),
