function runRes(result) {
    try {
        MrbrBase.mrbrInstance.loadManifest(Mrbr.Tests.Application$Radio[MrbrBase.MRBR_COMPONENT_MANIFEST])
            .then(_ => {

                let checkboxApplication = new Mrbr.Tests.Application$Radio()
            })
...
async function onReady() {
    try {
        const manifest =
            [
                Mrbr.IO.File.component(Mrbr.Tests.Application$Radio, 0),
